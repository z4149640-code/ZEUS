...
export default function ProductClient({ product, relatedProducts }: Props) {
  ...
  const imagesList = product.variants?.map((v) => v.imageUrl).filter(Boolean) || [];
  const currentImageUrl = activeVariant?.imageUrl;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {product.variants?.map((v, idx) => (
                <button
                  ...
                  className={`relative w-20 h-24 sm:w-24 sm:h-28 flex-shrink-0 rounded-sm border overflow-hidden transition-all ${activeVariantIndex === idx ? 'border-white bg-white/10' : 'border-white/10 bg-neutral-900/40 hover:border-white/40'}`}
                  title={v.colorName}
                >
                  <img src={v.imageUrl} alt={v.colorName} className="w-full h-full object-contain p-1" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-col pt-4 md:pt-10">
            ...
            {product.variants && product.variants.length > 0 && (
              <div className="mb-6">
                <span className="font-display text-sm uppercase tracking-[0.2em] text-white/60 block mb-3">
                  اختر اللون: <span className="text-white ml-1 font-body">{activeVariant?.colorName}</span>
                </span>
          ...
          >
            ...
            >
              {(size && activeVariant) ? "إضافة إلى العربة (ADD TO CART)" : "الرجاء اختيار المقاس"}
            </button>
            <button
